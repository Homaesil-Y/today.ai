...
| OPS-003 | 이메일 | 데일리 리포트 발송·기본 08:00 사용자 시간대 | DEV 20~21 | `report.ts`, `notify.ts`, `daily-email.yml` | P1 | ADAPTER_READY | 리포트 생성·공개·이메일 발송 코드 완료. 발송은 **Gmail API(OAuth2) 우선, Resend 폴백**(구독자 조회·당일 중복 방지·retry·notifications 기록). 매일 08:00 KST 워크플로. 자격증명(GMAIL_* 또는 RESEND) 미설정 시 실패 없이 blocked |
