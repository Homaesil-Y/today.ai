...
1. AI 관련성과 제품 의도를 규칙으로 필터링한다. Hacker News는 `Show HN` 제품 소개와 직접 GitHub repository 링크를 우선해 일반 기사·블로그 오탐을 줄인다. Product Hunt는 이미 출시된 제품이므로 제품 의도는 전제하고 AI 관련성(제목·태그라인·설명·토픽)만 검사하며, 공식 website URL을 대표 도메인으로 사용한다.
