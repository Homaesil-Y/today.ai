...
export default async function OnboardingPage({ searchParams }: { searchParams: Promise<{ next?: string }> }) {
  ...
  return <div className="onboarding-page"><section className="onboarding-card"><span className="brand-mark onboarding-brand"><Sparkles size={24} /></span><p className="eyebrow">WELCOME TO 오늘의 AI</p><h1>{profile?.display_name ? `${profile.display_name}님, ` : ""}관심 있는 AI 분야를 알려주세요.</h1><p className="onboarding-copy">선택한 분야를 중심으로 관심 목록과 알림을 구성합니다. 지금 선택하지 않아도 괜찮습니다.</p><form action={completeOnboarding}><input type="hidden" name="next" value={nextPath} /><div className="choice-grid onboarding-choices">{(categories ?? []).map((category) => <label className="choice-card" key={category.slug}><input type="checkbox" name="categories" value={category.slug} /><span>{category.name}</span></label>)}</div><div className="onboarding-alert"><Bell size={18} /><label><input type="checkbox" name="dailyDigest" defaultChecked />매일 아침 주요 AI 트렌드 요약 받기</label><input type="hidden" name="surgeAlert" value="on" /><input type="hidden" name="digestTime" value="08:00" /></div><button className="button button-primary onboarding-submit" type="submit">설정 완료하고 시작하기 <ArrowRight size={17} /></button></form></section></div>;
}
