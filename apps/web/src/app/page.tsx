...
import { collectionChannelsLabel } from "@/data/entity-sources";
...
const faqItems = (channels: string) => [
  { question: "오늘의AI는 어떤 서비스인가요?", answer: `${channels}의 공개 신호를 수집하고 최근 확산 속도를 분석해 주목할 AI 서비스를 한국어로 설명하는 트렌드 인텔리전스 서비스입니다.` },
...
export default async function DashboardPage() {
  ...
  // 수집 채널은 공개 중인 데이터에서 도출한다. 문구에 채널 이름을 적어두면 채널을 추가하거나
  // (Reddit처럼) 연동이 막힐 때 화면이 사실과 어긋난다.
  const channelsLabel = collectionChannelsLabel(allTrends.flatMap(({ sources }) => sources));
  ...
  const faq = faqItems(channelsLabel || "공개 채널");
  ...
  return (
    <div className="page">
      ...
      <section className="page-heading">
        <div><h1>오늘의 AI 트렌드</h1><p>{channelsLabel ? `${channelsLabel}에서 감지하고 검토를 통과한 AI 서비스입니다.` : "공개 채널에서 감지하고 검토를 통과한 AI 서비스입니다."}</p></div>
      ...
      <section className="section-block">
        ...
        {channelsLabel ? <div className="partial-notice" role="status"><Radio size={16} /><span><strong>데이터 안내</strong> 현재 순위는 {channelsLabel} 수집 신호를 기준으로 계산합니다.</span></div> : null}
      ...
      <section className="section-block answer-section" aria-labelledby="answer-heading">
        ...
        <div className="answer-grid">{faq.map(({ question, answer }) => <article className="panel" key={question}><h3>{question}</h3><p>{answer}</p></article>)}</div>
      </section>
    </div>
  );
}
