      ...
      <div className="mobile-ranking">
        {trends.map((trend) => (
          <Link href={`/services/${trend.slug}`} className="mobile-rank-card" key={trend.id}>
            <strong className="mobile-rank">{String(trend.rank).padStart(2, "0")}</strong>
            <span className="mobile-service"><strong>{trend.name}</strong><small>{trend.category} · {trend.signals[0]?.delta24h ? `+${trend.signals[0].delta24h.toLocaleString("ko-KR")}` : "초기 집계"}</small></span>
